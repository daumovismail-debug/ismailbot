...
## 🎥 Кино-композиция (язык кадра)
Каждый кадр строится по киноправилам (писать в промпт, англ.):
- **Планы:** extreme close-up (слёзы, деталь) · close-up (эмоция) ·
  medium (диалог) · wide/establishing (обстановка).
- **Ракурс под смысл:** eye-level (нейтраль) · **low angle** (герой силён/давит) ·
  **high angle** (герой подавлен/жертва) · over-the-shoulder (диалог, напряжение) ·
  **dutch tilt** (хаос, тревога).
- **Правило третей:** герой и его глаза — на линии верхней трети, не по центру.
- **Глубина:** передний/средний/задний план + shallow depth of field
  (герой резкий, фон в мягком боке).
- **9:16:** оставлять headroom сверху, **низ чистый под субтитр**, «воздух»
  по направлению взгляда (leading room).

## 🎬 Сложные мизансцены (3+ персонажа, движение)
- Расставлять героев **по глубине** (передний/средний/задний), НЕ в ряд.
- Главный в моменте — ближе/крупнее или на low angle (доминирует).
- Чёткие **взгляды** (кто на кого) и групповая динамика (кто против кого).
- Движение в кадре передавать позой и «смазом» (motion blur) при необходимости.
- Пример хвоста: `three characters staged in depth — wife foreground left
  (sharp), husband midground right, mother-in-law background in doorway
  (soft), tense triangle, low angle on wife, vertical 9:16.`
...
## 🔧 Профи-апгрейд (2026)
- **Идентичность + стиль на каждом кадре:** референс-лица (cref) + референс-стиля (sref).
- **LoRA на персонажа** (10–30 фото + триггер-слово) — самый крепкий лок (позже).
- **Цветовой скрипт/LUT** по сценам (напр. teal-orange) — эмоция и единый бренд.
- **Оптика:** указывать объектив/глубину резкости/фокус (напр. 85mm, shallow DoF).
- **3-точечный свет + цветовая температура** (key/fill/rim, 3200K/5600K, объёмный свет).
- **Сильные negative-prompts** на руки/лицо (лишние пальцы, кривые глаза, слипшиеся конечности).
- **seed-контроль** для повторяемости кадра.
...
8. План и **ракурс** подходят смыслу (low/high angle под силу/слабость)?
9. Композиция по правилу третей + глубина (передний/средний/задний план)?
10. В мультисценах — расстановка по глубине, взгляды, все персонажи из касты?
11. Фон/действие совпадают с кадром сценария?
